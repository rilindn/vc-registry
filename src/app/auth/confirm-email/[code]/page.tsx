...
export default function ConfirmEmail({ params }: { params: { code: string } }) {
  ...
  useEffect(() => {
    console.log('params.code === session?.user?.verificationCode', params.code, session?.user?.verificationCode)
    ...
    const fetchData = async () => {
      if (status === 'loading') {
        ...
        await updateUser({ email: session?.user?.email || '', updateData: {emailVerified: true}, replace: true })
        update()
        setConfirmationStatus('confirmed');
